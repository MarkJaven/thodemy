...
const resourceUpload = multer({
  storage: multer.memoryStorage(),
  limits: { fileSize: TOPIC_RESOURCE_MAX_FILE_SIZE_BYTES },
});
...
/**
 * Handle single file upload for topic resources (admin-uploaded learning materials).
 * @param {import("express").Request} req
 * @param {import("express").Response} _res
 * @param {import("express").NextFunction} next
 * @returns {void}
 */
const uploadTopicResource = (req, _res, next) => {
  resourceUpload.single("file")(req, _res, (err) => {
    if (err) {
      if (err.code === "LIMIT_FILE_SIZE") {
        return next(new BadRequestError("Resource file is too large."));
      }
      return next(new BadRequestError(err.message || "Unable to upload file."));
    }
    return next();
...
module.exports = { uploadTopicSubmission, uploadActivitySubmission, uploadTopicResource };
