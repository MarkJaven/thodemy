...
const BOOTCAMP_CRITERIA: {
...
}[] = [
  {
    ...
    items: [
      ...
      { key: "a2_problem_solving", label: "Problem-solving and Initiative", weight: 6 },
      { key: "a3_communication", label: "Communication Skills", weight: 6 },
      { key: "a4_leadership", label: "Team Leadership and Dynamics", weight: 6 },
    ],
  },
  {
    category: "B",
    label: "PRODUCTIVITY",
    items: [
      { key: "b1_efficiency", label: "Efficiency and Time Management", weight: 5 },
      { key: "b2_deadlines", label: "Meeting Deadlines", weight: 5 },
      { key: "b3_tools", label: "Utilization of Tools and Technologies", weight: 5 },
      { key: "b4_problem_solving", label: "Problem-solving and Adaptability", weight: 5 },
    ],
  },
  {
    category: "C",
  ...
  },
  {
    ...
    items: [
      { key: "d1_responsiveness", label: "Responsiveness and Availability", weight: 5 },
  ...
  },
  {
    ...
    label: "COMPLIANCE",
    items: [
      { key: "f1_policies", label: "Adherence to Policies and Procedures", weight: 5 },
      { key: "f2_reporting", label: "Timely and Accurate Reporting", weight: 5 },
    ],
  },
  {
    category: "G",
    ...
    items: [
      { key: "g1_integrity", label: "Integrity and Professionalism", weight: 5 },
...
const getBootcampStrengthKey = (category: string) =>
  `cat_${category}_strength`;
...
const EvaluationSection = () => {
  ...
  const [scoreboardModalError, setScoreboardModalError] = useState<string | null>(null);
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      setError(
        loadError instanceof Error ? loadError.message : "Unable to load evaluations."
      );
    } finally {
      setLoading(false);
    }
  ...
  const normalizeToFive = (score: number | null | undefined, maxScore?: number) => {
  ...
  const toWholeScoreOption = (
    ...
    return rounded >= 1 && rounded <= maxScore ? String(rounded) : "";
  };

  const getScoreboardEntries = () =>
    Array.from(pendingScores.values()).filter((entry) => entry.sheet === "scoreboard");

  const parseScoreboardEntry = (entry: Pick<ScoreInput, "criterion_key" | "category">) => {
    ...
    if (rawKey.includes(SCOREBOARD_ACTIVITY_KEY_SEPARATOR)) {
      ...
      return {
        ...
        rubricKey: SCOREBOARD_CRITERIA_KEY_SET.has(rubricKey) ? rubricKey : null,
      };
    }
  ...
  const getScoreboardActivities = (): ScoreboardActivity[] => {
    ...
    const list = Array.from(activities.values()).map((activity) => {
      const scoredNormalized = SCOREBOARD_CRITERIA.map((criterion) => {
        ...
        const criterionMax =
          activity.maxScores[criterion.key] ?? getCriterionMaxScore(criterion.key);
    ...
    list.sort((left, right) => left.activityLabel.localeCompare(right.activityLabel));
    return list;
  };
  ...
  const saveScores = async () => {
    ...
    try {
      ...
      setActionError(
        saveError instanceof Error ? saveError.message : "Unable to save scores."
      );
    } finally {
  ...
  const handleDelete = async (evalId: string) => {
    ...
    try {
      ...
      setActionError(
        delError instanceof Error ? delError.message : "Unable to delete evaluation."
      );
    }
  };
  ...
  const openScoreboardGradeModal = (activity: ScoreboardActivity) => {
    ...
    for (const criterion of SCOREBOARD_CRITERIA) {
      ...
      criteriaScores[criterion.key] = toWholeScoreOption(existingScore, criterionMax);
    }
  ...
  const applyScoreboardGrade = () => {
    ...
    const missingCriteria = SCOREBOARD_CRITERIA.filter(
      (criterion) =>
        !String(scoreboardModalDraft.criteriaScores[criterion.key] || "").trim()
    );
    ...
    for (const criterion of SCOREBOARD_CRITERIA) {
      ...
      if (!/^\d+$/.test(rawValue)) {
        setScoreboardModalError(
          `Score for ${criterion.label} must be a whole number (1-${safeMax}).`
        );
        return;
      }

      const safeScore = Number.parseInt(rawValue, 10);
      if (safeScore < 1 || safeScore > safeMax) {
        setScoreboardModalError(
          `Score for ${criterion.label} must be between 1 and ${safeMax}.`
        );
        return;
      }
  ...
  const computeCriterionScoreFromScoreboard = (
    ...
    const entries = getScoreboardEntries().filter(
      (entry) => entry.category === criterionKey || entry.criterion_key === criterionKey
    );
    ...
    return normalized.reduce((sum, score) => sum + score, 0) / normalized.length;
  };
  ...
  const getQuizRatingLabel = (rating: number): string => {
    switch (rating) {
      ...
      case 1: return "Not yet capable";
      case 2: return "Some capability";
      case 3: return "Needs improvement";
      case 4: return "Capable";
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 rounded-xl border border-white/10 bg-ink-800 p-4 sm:grid-cols-4">
          <div>
            ...
            <p className="text-sm text-white">
              {(selectedEval.trainee_info as Record<string, string>)?.position || "â€”"}
            </p>
          </div>
          <div>
            <p className="text-xs text-slate-400">Department</p>
            <p className="text-sm text-white">
              {(selectedEval.trainee_info as Record<string, string>)?.department || "â€”"}
            </p>
          </div>
          <div>
            <p className="text-xs text-slate-400">Period</p>
            <p className="text-sm text-white">
              {selectedEval.period_start || "â€”"} to {selectedEval.period_end || "â€”"}
            </p>
          </div>
          <div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          >
            {savingScores ? "Processing..." : "Auto-populate from Quiz/Activity"}
          </button>
          <button
            onClick={saveScores}
            disabled={savingScores}
        ...
        >
          ...
          {scoreboardModalDraft && (
            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-sm text-slate-300">Activity</label>
              ...
              <div>
                <label className="mb-1 block text-sm text-slate-300">Remarks</label>
                <input
                  type="text"
  ...
  function renderScorecard() {
    ...
    const quizAvgRating = quizAvgEquivalent !== null ? computeQuizRating(quizAvgEquivalent) : 0;
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10 text-left text-xs text-slate-400">
                      ...
                      <th className="px-2 py-2 font-medium w-28">Reflected Score</th>
                      <th className="px-2 py-2 font-medium w-28">Mapped Activities</th>
                    </tr>
                  </thead>
                  <tbody>
                    {group.items.map((item) => {
                      const score = computeCriterionScoreFromScoreboard(item.key);
        ...
        <div className="rounded-lg border border-yellow-500/30 bg-yellow-900/10 p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            {quizAvgEquivalent !== null && (
              <span className="text-sm text-yellow-300">
                Avg Equivalent: {quizAvgEquivalent.toFixed(1)} | Rating: {quizAvgRating} ({getQuizRatingLabel(quizAvgRating)})
              </span>
            )}
          </div>
          <p className="mb-3 text-xs text-slate-400">
            Auto-populated from quiz scores. Equivalent = (Score / Total Items) * 50 + 50. Rating is computed from the equivalent.
          </p>
          ...
          {quizGrades.length > 0 ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10 text-left text-xs text-slate-400">
                    ...
                    <th className="px-2 py-2 font-medium w-16 text-center">Rating</th>
                    <th className="px-2 py-2 font-medium w-20 text-center">Score</th>
                    <th className="px-2 py-2 font-medium w-24 text-center">Equivalent</th>
                    <th className="px-2 py-2 font-medium w-24 text-center">Total Items</th>
                  </tr>
                </thead>
                <tbody>
                  {quizGrades.map((entry, index) => {
                    const equivalent = computeQuizEquivalent(entry.score, entry.max_score);
                    const rating = computeQuizRating(equivalent);
                    return (
                      <tr key={entry.criterion_key} className="border-b border-white/5">
            ...
            <div className="rounded-lg border border-white/5 px-4 py-6 text-center text-sm text-slate-500">
              No quiz grades yet. Click "Auto-populate from Quiz/Activity" to pull quiz scores.
            </div>
          )}
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {BOOTCAMP_CRITERIA.map((group) => {
                  ...
                  const improvementKey = getBootcampImprovementKey(group.category);

                  return (
                    <tr key={`feedback-${group.category}`} className="border-b border-white/5">
                      <td className="px-2 py-2 text-white">
                        {group.category}. {group.label}
                      </td>
                      <td className="px-2 py-2 text-slate-300">
                      ...
                      <td className="px-2 py-2">
                        <textarea
                          rows={2}
                          value={getRemarks(BOOTCAMP_ENDORSEMENT_FEEDBACK_SHEET, strengthKey)}
                          onChange={(e) =>
                            setRemarksValue(
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-lg border border-yellow-500/30 bg-yellow-900/10 p-4">
          ...
          {quizGrades.length > 0 ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10 text-left text-xs text-slate-400">
                    ...
                    <th className="px-2 py-2 font-medium w-24 text-right">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {quizGrades.map((entry, index) => {
                    ...
                    return (
                      <tr key={entry.criterion_key} className="border-b border-white/5">
                        ...
                        <td className="px-2 py-2 text-right">
                          {quizEntries.find((a) => a.activityKey === entry.criterion_key) && (
                            <button
                              onClick={() => {
                                const match = quizEntries.find(
                                  (a) => a.activityKey === entry.criterion_key
                                );
                                if (match) openScoreboardGradeModal(match);
                              }}
                              className="mr-2 rounded border border-purple-500/40 px-2 py-1 text-xs text-purple-300 hover:bg-purple-500/10"
                              title="Override with criteria grading"
                            >
                              Grade
                            </button>
                          )}
                          <button
                            onClick={() => handleDeleteScoreboardActivity(entry.criterion_key)}
            ...
            <div className="rounded-lg border border-white/5 px-4 py-6 text-center text-sm text-slate-500">
              No quiz scores yet. Click "Auto-populate from Quiz/Activity" to pull quiz scores.
            </div>
          )}
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10 text-left text-xs text-slate-400">
                    ...
                    <th className="px-2 py-2 font-medium w-40">Criteria Scored</th>
                    <th className="px-2 py-2 font-medium w-24">Average</th>
                    <th className="px-2 py-2 font-medium w-28">Status</th>
                    <th className="px-2 py-2 font-medium w-20">Source</th>
                    <th className="px-2 py-2 font-medium w-40 text-right">Action</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    >
                      ...
                      <td className="px-2 py-2 text-right">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleDeleteScoreboardActivity(activity.activityKey);
                          }}
                          className="rounded border border-red-500/40 px-2 py-1 text-xs text-red-300 hover:bg-red-500/10"
...
function renderPerformance() {
    const perfCriteria = [
      { key: "pe_a", label: "Category A - Employee Engagement", weight: 25 },
      { key: "pe_b", label: "Category B - Productivity", weight: 20 },
      { key: "pe_c", label: "Category C - Work Quality", weight: 15 },
      { key: "pe_d", label: "Category D - Customer Satisfaction", weight: 10 },
      { key: "pe_e", label: "Category E - Self Improvement", weight: 10 },
      { key: "pe_f", label: "Category F - Compliance", weight: 10 },
      { key: "pe_g", label: "Category G - Ethics and Values", weight: 10 },
    ];
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-purple-500/30 bg-purple-900/10 p-6 text-center">
          ...
          <p className="mt-2 text-xs text-slate-400">
            Bootcamp: {bootcampPercent.toFixed(1)}% | Endorsed: {performancePercent.toFixed(1)}%
          </p>
        </div>
        ...
        <div>
          ...
          {(() => {
            ...
            if (sbEntries.length === 0)
              return (
                <p className="text-sm text-slate-400">No scoreboard data yet.</p>
              );
